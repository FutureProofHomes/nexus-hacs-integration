...
- `__init__.py`: Main integration setup with deprecated `generate_content` service
...
- `config_flow.py`: Configuration flow for setup (includes subentry flows, zeroconf discovery)
- `const.py`: Constants and configuration options

### Services
One **deprecated** service kept for backward compatibility:
...
- `CONF_REASONING_SUMMARY`: Reasoning summary mode (default: "auto")
- `CONF_CODE_INTERPRETER`: Enable code interpreter (default: False)
- `CONF_WEB_SEARCH`: Enable web search (default: False)
- `CONF_WEB_SEARCH_USER_LOCATION`: Use approximate user location for search
- `CONF_WEB_SEARCH_CITY`, `REGION`, `COUNTRY`, `TIMEZONE`: Location fields for web search
- `CONF_WEB_SEARCH_INLINE_CITATIONS`: Include source citations in search results
- `CONF_WEB_SEARCH_CONTEXT_SIZE`: Search context size ("low", "medium", "high")
- `CONF_STORE_RESPONSES`: Persist responses to history (default: False)
- `CONF_SERVICE_TIER`: Service tier selection ("auto", "flex", "default", "priority")
- `CONF_VERBOSITY`: Text verbosity for gpt-5 models ("low", "medium", "high")
